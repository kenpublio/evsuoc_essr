...
<div id="verificationModal" class="verification-modal" style="display: none;">
    <div class="verification-modal-content">
        <div class="verification-modal-header">
            ...
            <button type="button" class="close-modal" onclick="closeVerificationModal()">&times;</button>
        </div>
        <div class="verification-modal-body">
            <p>A verification code has been sent to: <strong id="verificationEmail"></strong></p>
            <p>Please enter the 6-digit code below to confirm your email address.</p>
            
            <div class="code-inputs">
                <input type="text" maxlength="1" class="code-digit" data-index="0" onkeyup="moveToNext(this, 0)">
                <input type="text" maxlength="1" class="code-digit" data-index="1" onkeyup="moveToNext(this, 1)">
                <input type="text" maxlength="1" class="code-digit" data-index="2" onkeyup="moveToNext(this, 2)">
                <input type="text" maxlength="1" class="code-digit" data-index="3" onkeyup="moveToNext(this, 3)">
                <input type="text" maxlength="1" class="code-digit" data-index="4" onkeyup="moveToNext(this, 4)">
                <input type="text" maxlength="1" class="code-digit" data-index="5" onkeyup="moveToNext(this, 5)">
            </div>
            ...
            <p class="timer-text">Code expires in: <span id="timer">10:00</span></p>
        </div>
    </div>
</div>
...
function getVerificationCode() {
    let code = '';
    for (let i = 0; i < 6; i++) {
        const digit = document.querySelector(`.code-digit[data-index="${i}"]`).value;
        if (digit) code += digit;
    }
    return code;
}
...
function verifyCode() {
    ...
    if (code.length !== 6) {
        document.getElementById('verificationMessage').innerHTML = '<span style="color: #dc3545;">Please enter the 6-digit code</span>';
        return;
    }
    ...
    .then(data => {
        if (data.success) {
            emailVerified = true;
            document.getElementById('verificationMessage').innerHTML = '<span style="color: #28a745;"><i class="fas fa-check-circle"></i> Email verified successfully!</span>';
            setTimeout(() => {
                closeVerificationModal();
            }, 1500);
        } else {
            document.getElementById('verificationMessage').innerHTML = `<span style="color: #dc3545;">${data.message}</span>`;
        }
    })
    .catch(error => {
        document.getElementById('verificationMessage').innerHTML = '<span style="color: #dc3545;">Verification failed. Please try again.</span>';
    });
}
...
function clearCodeInputs() {
    for (let i = 0; i < 6; i++) {
        document.querySelector(`.code-digit[data-index="${i}"]`).value = '';
    }
...
// Update form submission to check email verification
document.getElementById('registerForm')?.addEventListener('submit', function(e) {
    ...
    if (!emailVerified) {
        e.preventDefault();
        alert('Please verify your email address first. A verification code has been sent to your email.');
        return false;
    }
