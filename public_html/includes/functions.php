...
public function getRegistrarOffice() {
     ...
     */
     ...
     public function getServicesByOfficeId($office_id) {
        $stmt = $this->conn->prepare("SELECT * FROM service_types WHERE is_active = 1 ORDER BY name");
        $stmt->execute();
        $result = $stmt->get_result();
        return $result->fetch_all(MYSQLI_ASSOC);
    }
    public function getUserById($id) {
        $stmt = $this->conn->prepare("SELECT id, username, email, role, student_id, fullname, profile_picture, is_active, created_at, last_login FROM users WHERE id = ?");
